...
const Layout = ({ children, activeTab, onTabChange }: LayoutProps) => {
  ...
  const tabs = [
    ...
    { id: 'internships', label: 'Experience', icon: Award },
    { id: 'co-curricular', label: 'Co-Curricular', icon: Book },
    { id: 'extra-curricular', label: 'Extra-Curricular', icon: Star },
  ];
  ...
  return (
    <div className="min-h-screen flex w-full relative">
      ...
      <div 
        ...
        style={{
          background: `
            radial-gradient(ellipse 400px 400px at ${mousePosition.x}% ${mousePosition.y}%, 
              rgba(147, 51, 234, 0.35) 0%, 
              rgba(99, 102, 241, 0.2) 30%, 
              transparent 70%
            ),
            radial-gradient(ellipse 600px 600px at ${100 - mousePosition.x}% ${100 - mousePosition.y}%, 
      ...
      <main className="flex-1 h-screen overflow-hidden">
        <div className={`h-full ${activeTab === 'home' ? 'p-4 sm:p-6 lg:p-8 overflow-hidden' : 'overflow-auto'}`}>
          <div className={activeTab === 'home' ? 'h-full' : 'p-4 sm:p-6 lg:p-8'}>
            {children}
          </div>
        </div>
      </main>
